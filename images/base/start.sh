...
if [ "$(id -u)" == 0 ]; then
	...
	# - CHOWN_HOME_EXCLUDE: exclude pattern for fd ex: "{nas,.profile,my_custom_folder/*}"
	...
	if [[ ${CHOWN_HOME} == "1" || ${CHOWN_HOME} == "yes" ]]; then
		...
		# Only run chown on files/directories not owned by NB_USER
		if [ -n "${CHOWN_HOME_EXCLUDE}" ]; then
			fd -E ${CHOWN_HOME_EXCLUDE} -o "!${NB_UID}" --full-path "/home/${NB_USER}" -H -x chown ${CHOWN_HOME_OPTS} "${NB_UID}:${NB_UID}" {}
		else
			fd -o "!${NB_UID}" --full-path "/home/${NB_USER}" -H -x chown ${CHOWN_HOME_OPTS} "${NB_UID}:${NB_UID}" {}
		fi
	fi
